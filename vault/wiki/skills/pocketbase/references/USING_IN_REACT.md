...
Auth is already scaffolded. Enabling PocketBase created these two files alongside
the client — read them before writing any auth code, and do NOT rewrite them from
scratch:

- `apps/web/src/contexts/AuthContext.jsx` — exports `AuthProvider` and `useAuth()`,
  which gives you `{ user, isAuthed, login, signup, logout }`. `user` tracks
  `pb.authStore` and re-renders on login/logout.
- `apps/web/src/components/ProtectedRoute.jsx` — redirects to `/login` when signed out.

Extend them in place when a task needs more (roles, profile loading, permissions).
Never add a second parallel hook — no `hooks/useAuth.js`, `lib/useAuth.js`, or
`lib/auth.jsx`. On a project created before this scaffolding shipped the two files
may be missing; in that case create them at exactly those two paths.
...
await signup(email, password, { name }); // extra fields are optional
```

Wrap the router once in `App.jsx` with `<AuthProvider>`, then guard the pages that
...
Both files import `@/lib/pocketbaseClient`, so they only exist once PocketBase is
enabled — never create them in a project without PocketBase.
...
in PocketBase SDK v0.21+.** It is NOT synchronous. Use the canonical pattern
below; never treat the return value as a sync function.
...
useEffect(() => {
  void pb
    ...
    .catch((error) => console.error("Realtime subscription failed", error));
...
Keep effect dependencies limited to values that change the subscription scope
(for example, the collection or authenticated user id). Put rapidly changing
callback state in refs so renders do not repeatedly reconnect the shared
realtime client.

### ❌ WRONG — don't do this

```jsx
// BROKEN: `subscribe` returns a Promise, not an unsubscribe function.
const unsub = pb.collection("tasks").subscribe("*", handler);
return () => unsub();
```

Calling that Promise during unmount throws `TypeError: unsub is not a function`
and can blank the app.
...
Uploads are written to the PocketBase environment serving the request. An
upload made from the preview domain is stored only in preview `pb_data`; it is
not copied to the production PocketBase database or file storage when the site
is published. If the same content must be deployed as initial production data,
bundle the source asset outside `pb_data/` and attach it from a new timestamped
migration.
...
- **Subscribing and not unsubscribing** — the effect cleanup MUST call
  `unsubscribe(...).catch(() => {})`, otherwise you get double updates after
  re-mounts or an unhandled rejection when cleanup races a reconnect.
