...
## Class names — three do NOT match the type string

The class name is usually the type string capitalised (`text` → `TextField`),
but for three types it is not. These are the most common migration failures:

| Type string | Class        | Does NOT exist  |
| ----------- | ------------ | --------------- |
| `bool`      | `BoolField`  | `BooleanField`  |
| `json`      | `JSONField`  | `JsonField`     |
| `url`       | `URLField`   | `UrlField`      |

Using a name that isn't bound throws `ReferenceError: JsonField is not defined`,
which aborts the whole migration batch — PocketBase then refuses to start, so
the site stays down until the file is fixed.

The complete set, and there is no other field class:

`Field`, `TextField`, `EditorField`, `NumberField`, `BoolField`, `EmailField`,
`URLField`, `DateField`, `AutodateField`, `SelectField`, `RelationField`,
`FileField`, `JSONField`, `GeoPointField`, `PasswordField`

`Field` is the odd one out: it is not "the class for an untyped field" but a
generic constructor that resolves the concrete class from the object's `type`
key. So `new Field({ name: "meta", type: "json" })` is a valid argument to
`fields.add()`, while omitting `type` yields nothing usable — and it does not
throw there, so the failure only surfaces later at `add()`. Prefer the concrete
class when you know the type.

## Object literal vs class — where each is allowed

These are NOT interchangeable. An object literal is valid only inside the
`new Collection({ fields: [...] })` constructor, which parses it as JSON.
`collection.fields.add()` takes a Go `core.Field` value and rejects a plain
object with `could not convert [object Object] to core.Field` — again aborting
the batch.

```js
// ✅ Literal — inside the constructor's fields array (creating a collection)
new Collection({
  type: "base",
  name: "posts",
...
// ✅ Instance — adding to a collection that already exists
collection.fields.add(new TextField({ name: "title", required: true, max: 200 }));

// ❌ Literal passed to add() — always fails
collection.fields.add({ name: "title", type: "text" });
```
