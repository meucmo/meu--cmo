...
`add()` needs a field INSTANCE — an object literal there fails with
`could not convert [object Object] to core.Field`.

The constructor usually matches the type string, but NOT for three of them:
`bool` → `BoolField` (not `BooleanField`), `json` → `JSONField` (not
`JsonField`), `url` → `URLField` (not `UrlField`). The full set:
`Field`, `TextField`, `NumberField`, `BoolField`, `EmailField`, `URLField`,
`DateField`, `AutodateField`, `SelectField`, `FileField`, `JSONField`,
`RelationField`, `EditorField`, `GeoPointField`, `PasswordField`.
See `FIELD_TYPES.md` → "Class names".
