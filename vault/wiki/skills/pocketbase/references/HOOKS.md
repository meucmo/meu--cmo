...
PocketBase runs each hook callback in an isolated JSVM scope. Keep callbacks
self-contained because they cannot reliably access variables or helper
functions declared in the file's outer scope. Define every custom helper and
value inside the callback. PocketBase globals such as `$app` are safe.
...
Wrap best-effort email and HTTP notifications in `try/catch` so their failure
cannot abort the record operation.
...
> **Never strip `/hcgi/platform`.** `meta.appURL` already includes the site's
> required `/hcgi/platform` reverse-proxy prefix, and the built-in
> `/_/#/auth/confirm-password-reset/<token>` page 404s without it. Do not "fix"
> or remove `/hcgi/platform` from a URL even while debugging a 404 — a 404 here
> is almost never the prefix: check for a custom page or wrong route first, and
> confirm the site is published. Keep the built-in reset page; never replace it
> with a custom `/reset-password` route.
...
- **Configuring SMTP** — not needed by default; the platform relay handles
  delivery with no setup. Only when the user **explicitly** asks to send from
  their own address (their Hostinger mail, Gmail, custom-domain SMTP — often
  because the default sender lands in spam), enable custom SMTP via a
  `settings.smtp` migration. It cannot be set from the API/dashboard/CLI — see
  `pocketbase/references/CUSTOM_SMTP.md`.
...
- **Hook-name plurality** — the plural-prefix hooks are
  `onCollectionsImportRequest`, `onCollectionsListRequest`,
  `onRecordsListRequest`, `onSettingsListRequest`, `onSettingsReload`, and
  `onSettingsUpdateRequest`. Other collection and record hooks use singular.
- **File extension** — must end in `.pb.js`. Plain `.js` is silently ignored.
- **Mutating JSON values directly** — PB may expose JSON objects/arrays as
  Go-backed values. Clone with `JSON.parse(JSON.stringify(value))` before
  changing them.
