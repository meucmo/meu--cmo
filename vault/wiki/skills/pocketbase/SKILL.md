...
2. **Writes = files. Reads = injected schema or read-only API snapshot.**
   ...
   - If you need a fresh schema snapshot mid-work (for example after
     ...
     missing/ambiguous), call `inspect_pocketbase` with
     `include: ["schema"]`.
   - Before changing any existing migration file, call `inspect_pocketbase`
     with `include: ["migrations"]`. `applied` and `pending` come from the
     preview database's `_migrations` table; `recent_failures` is log evidence
     and may be stale. Never treat a recent failure alone as proof that a file
     is safe to rewrite.
   - Normally, an applied migration is immutable: add a new timestamped delta
     migration. Repair an existing file only when you created it during the
     current task, it is still pending, and no successful `reload_app` applied
     it. An incoming `<detected-error source="u4s-production">` that names the
     exact failed migration is the only other exception. Inspection shows the
     preview database only; infer the smallest compatibility repair from the
     production error and preview state, edit only that named file, and preserve
     the final schema already reached in preview.
...
### Preview and production data are separate

The preview sandbox and the published website use different PocketBase
databases and file storage. Runtime writes made through the SDK or dashboard
only affect the environment handling that request. For example, an artwork
row and image uploaded from a Studio page on the preview domain exist only in
the preview sandbox.

Publishing copies application source, including migrations and hooks, but
excludes `apps/pocketbase/pb_data/`. It therefore does **not** copy preview
records or uploaded files into production. Never tell the user that publishing
will transfer runtime PocketBase data.

If content must be reproducible in a fresh or published database, add it with
a new timestamped migration and keep any required source assets outside
`pb_data/` in the versioned application source. Do not use `onBootstrap` hooks
to synchronize or seed environments; hooks run on every PocketBase start and
are for event-driven server behavior, not deployment-time data transfer.
...
- PocketBase request/error log table: `_logs`
  ...
  which filters out noisy successful realtime requests. Before changing code
  for a PB 400/404, call `inspect_pocketbase` with `include: ["errors"]` and
  verify the schema fields, auth state, access rules, and record existence. A
  404 can mean either a missing record or a denied access rule. Useful fields
  are `message`, `data.status`, `data.error`, `data.details`, `data.url`, and
  `data.auth`.

`inspect_pocketbase` is the single read-only diagnostic tool. Select only the
sections you need: `schema`, `errors`, and/or `migrations`. The schema section
reads `pb_data/data.db` directly and intentionally excludes `_superusers`. The
migrations section reports migration files, exact applied/pending status for
the preview database, applied history whose source file is missing, and recent
PM2 failure evidence. Never call `migrations:revert`: it executes destructive
down logic and is reserved for the customer-initiated message-revert flow. Do
not use ad-hoc `bash` commands as a status check; use the inspection tool.
...
| `pocketbase/references/CUSTOM_SMTP.md`    | User EXPLICITLY wants mail sent from their own address (their Hostinger mail / Gmail / custom-domain SMTP), or "emails go to spam from the default sender" |
...
- **Guessing a field class name.** The class is usually the type string
  capitalised, but three are NOT: `bool` → `BoolField`, `json` → `JSONField`,
  `url` → `URLField`. `BooleanField`, `JsonField` and `UrlField` do not exist
  and throw `ReferenceError: <name> is not defined`, aborting the migration.
  The complete set: `Field`, `TextField`, `EditorField`, `NumberField`,
  `BoolField`, `EmailField`, `URLField`, `DateField`, `AutodateField`,
  `SelectField`, `RelationField`, `FileField`, `JSONField`, `GeoPointField`,
  `PasswordField`.
- **Passing an object literal to `fields.add()`.** A literal describes a field
  only inside the `new Collection({ fields: [...] })` constructor. Adding to a
  collection that already exists needs an INSTANCE, or you get
  `could not convert [object Object] to core.Field`.

  ```js
  // ❌ WRONG — aborts the migration
  collection.fields.add({ name: "meta", type: "json" });

  // ✅ CORRECT
  collection.fields.add(new JSONField({ name: "meta" }));
  ```

- NEVER use `record.getId()` — use `record.id`.
- NEVER use `record.save()` — records do not save themselves; use
  `app.save(record)`.
- NEVER use `app.findRecordsByCollection()` — it does not exist; use
  `app.findAllRecords()` or `app.findRecordsByFilter()`.
- NEVER use `record.setUsername()` — removed in v0.23; `username` is not a
  built-in auth field.
- **Treating a field's metadata as a property.** `field.type` is a METHOD, so
  `field.type === "relation"` is always false. Call `field.type()`.
