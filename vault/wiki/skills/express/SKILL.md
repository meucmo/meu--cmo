...
- A secret the USER must supply that isn't set yet (a payment/API key they
  paste) → `respondNotConfigured(...)`, NOT a throw. See "User-supplied
  secrets" below.
- Every HTTP-failure throw must include BOTH `${res.status}` and
  `${res.statusText}` in the message — never `${res.status}` on its own.

## User-supplied secrets: report "not configured", don't 500

A secret the **user** must supply — a payment key (`STRIPE_SECRET_KEY`,
`PAYPAL_CLIENT_SECRET`, `SQUARE_ACCESS_TOKEN`), a data-API key
(`TMDB_API_KEY`, `SERPAPI_KEY`), `TWILIO_AUTH_TOKEN`, SMTP creds, etc. — is
often **not set yet** because the user has not pasted it. This is NOT a server
error. If you `throw` on it, `errorMiddleware` returns a 500 on every page
load, the platform reads the site as broken, and the fix can never succeed
because the AI cannot invent the user's private key. (Contrast: Integrated AI
provider keys are provisioned by the platform — never ask for those.)

Handle a missing user secret as a first-class **setup state**, not an error:

1. Use `apps/api/src/utils/integrationConfig.js`. When the key is missing,
   return `respondNotConfigured(...)` (a 503 carrying the
   `INTEGRATION_NOT_CONFIGURED` marker) instead of throwing.
2. Add a small status route (e.g. `GET /stripe/status`) returning
   `{ configured: boolean }`, and have the frontend check it **before** the
   action call. Render a calm "not set up yet" state — do NOT fire the action
   fetch on mount when unconfigured (that is what produces the error).
3. In your reply, tell the user exactly which key(s) to add (it is their own
   account's key). Once they provide it, add it to `apps/api/.env` and
   `reload_app`.

Genuine failures (a real upstream error, a bug, a platform value that should
always be present) still `throw` — this exception is ONLY for a secret the
user is expected to supply and has not yet.

```js
// apps/api/src/routes/stripeStatus.js — register as GET /stripe/status
import { isIntegrationConfigured } from "../utils/integrationConfig.js";

export default (req, res) => {
  res.json({ configured: isIntegrationConfigured("STRIPE_SECRET_KEY") });
};
```

```js
// apps/api/src/routes/stripeCheckout.js — register as POST /stripe/checkout
...
export default async (req, res) => {
  if (!isIntegrationConfigured("STRIPE_SECRET_KEY")) {
    return respondNotConfigured(res, { integration: "Stripe", envKeys: "STRIPE_SECRET_KEY" });
  }

  // ...real logic. Genuine upstream/server failures still throw — never return a 5xx.
};
```
...
export default async (req, res) => {
  ...
  // NOTE: if EXAMPLE_API_KEY is a secret the USER must supply, do NOT throw —
  // return respondNotConfigured(...) instead (see "User-supplied secrets").
  if (!EXAMPLE_API_KEY) {
