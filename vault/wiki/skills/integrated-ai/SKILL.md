...
login", "no sign-up", "without an account", "anyone can use it") — this applies to
...
LOGIN-REQUIRED. A customer-facing, sales, support, or marketing assistant is NOT
an explicit request for public — do NOT infer public access from the site type,
the audience, or "less friction / better UX". Only the user's literal words
("no login", "no sign-up", "anyone can use it", "public") count. When the user did ask for public, remove BOTH lines from
...
"no sign-up", "without an account", "anyone can use it", "public"). **Always state
